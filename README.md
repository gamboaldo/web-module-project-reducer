...
- [ z] Note the `ADD_ONE` action case (in ./reducer/index.js) and `addOne` action creator (in ./actions/index.js). This action adds 1 to our total.
- [ z] Import the `addOne` action creator into App.js.
- [ z] Within `App.js`, create an event handler connected to the 1 button's `onClick` method.
- [z ] Within your event handler, dispatch the `addOne` action creator.
...
- [s ] Import in your new action creator into `App.js.`
...
- [ d] When `M+` is pressed, the current memory value should be set to the current total value. Test by seeing the result of memory in the UI.
- [d ] When `MR` is pressed, the current memory value should be applied to the current total value(See the APPLY_NUMBER case). Test by adding a value to memory and then seeing if the total updates correctly when pressed.
- [d ] When `MC` is pressed, the current memory value should be set to zero. Test by adding a value to memory and then seeing the memory value reset to zero when pressed.
